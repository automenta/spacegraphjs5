...
export class UIManager {
    ...
    _onSelectionChanged = payload => {
        const selectedEdges =
            payload.selected.size > 0 && payload.type === 'edge' ? payload.selected : new Set();
    ...
    _onDocumentClick = e => {
        ...
        if (this.hudManager.keyboardShortcutsDialog.keyboardShortcutsDialogElement?.contains(e.target))
            return;
        if (this.hudManager.layoutSettingsDialog.layoutSettingsDialogElement?.contains(e.target))
            return;
        ...
        if (this.edgeMenu.edgeMenuObject) {
            ...
            const clickedSelectedEdge =
                targetInfo.intersectedEdge && selectedEdges?.has(targetInfo.intersectedEdge);
    ...
    _getTargetInfo(event) {
        ...
        const needsRaycast =
            !resizeHandle && !nodeControlsButton && !contentEditableEl && !interactiveEl;
