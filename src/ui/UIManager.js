...
export class UIManager {
  ...
  _exitCurrentState() {
    switch (this.currentState) {
      ...
      case InteractionState.PANNING:
        ...
        this.container.style.cursor = "grab";
        break;
      case InteractionState.ORBITING:
        this.space.plugins.getPlugin("CameraPlugin")?.endOrbitDrag();
        this.container.style.cursor = "grab";
        break;
  ...
  _enterOrbitingState() {
    this.space.plugins
      .getPlugin("CameraPlugin")
      ?.startOrbitDrag(this.pointerState.clientX, this.pointerState.clientY);
    this.container.style.cursor = "grabbing";
  }
  ...
  _onPointerDown = (e) => {
    ...
    if (this.pointerState.button === 1 || this.pointerState.button === 2) {
      if (cameraMode === "drag_orbit") {
        e.preventDefault();
        this._transitionToState(InteractionState.ORBITING);
  ...
  _onPointerMove = (e) => {
    ...
    switch (this.currentState) {
      ...
      case InteractionState.ORBITING:
        e.preventDefault();
        this.space.plugins.getPlugin("CameraPlugin")?.orbitDrag(dx, dy);
        break;
  ...
  _onPointerUp = (e) => {
    ...
    if (!this.pointerState.isDraggingThresholdMet && e.button === 0) {
      ...
      if (
        ...
        // Intentionally left empty - placeholder for future implementation
      }
    }
  ...
  _handleNodeControlButtonClick(buttonEl, node) {
    ...
    switch (action) {
      ...
      case "shrink":
        ...
        break;
      default:
      // Warn when unknown node control action is encountered
  ...
  _onContextMenu = (e) => {
    e.preventDefault();
    if (this.currentState === InteractionState.ORBITING) return;
