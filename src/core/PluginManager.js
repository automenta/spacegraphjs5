...
export class PluginManager {
  ...
  add(plugin) {
    ...
    if (this.plugins.has(pluginName)) {
      console.warn(
        `PluginManager: Plugin "${pluginName}" already registered. Overwriting.`,
      );
    }
   ...
   */
  async initPlugins() {
    const pluginInitPromises = [...this.plugins.values()].map(
      async (plugin) => {
        try {
          await plugin.init?.();
          ...
          console.error(
            `Failed to initialize plugin ${plugin.getName()}:`,
            error,
          );
    ...
    if (failedPlugins.length > 0) {
      throw new Error(
        `Failed to initialize ${failedPlugins.length} plugin(s): ${failedPlugins.map((p) => p.plugin.getName()).join(", ")}`,
      );
    }
  }
