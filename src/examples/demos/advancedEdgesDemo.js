...
function createAdvancedDynamicEdges(space, basePosition = {x: 0, y: 0, z: 0}) {
    ...
    const animateAdvEdges = () => {
        ...
        edges.beziers.forEach((edge, index) =>
            edge.setCurveTension?.(Math.max(0.1, 0.4 + Math.sin(animationTime * 0.3 + index * 0.5) * 0.2))
        );
...
function createBasicStyledEdges(space, basePosition = {x: 0, y: -600, z: 0}) {
    ...
    nodePositions.forEach((pos, i) => {
        nodes.push(
            space.createNode({
                ...
                data: {...commonProps.data, label: `N${i + 1}`, color: S.Utils.randomColor()},
                ...commonProps,
            })
        );
    });
    ...
    space.addNode(
        new S.TextMeshNode(
            'bs-title',
            {x: basePosition.x, y: basePosition.y + 300, z: basePosition.z},
...
function createSpecializedStaticEdges(space, basePosition = {x: 800, y: -600, z: 0}) {
    ...
    const n4 = space.createNode({
        id: 'ssen4',
        position: {x: basePosition.x + 150, y: basePosition.y - 50, z: basePosition.z},
        data: {...commonProps.data, label: 'SSE4 (Val:75)', value: 75, color: 0xffcc00},
        ...commonProps,
    });
    ...
    space.addNode(
        new S.TextMeshNode(
            'sse-title',
            {x: basePosition.x, y: basePosition.y + 150, z: basePosition.z},
