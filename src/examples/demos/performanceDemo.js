...
function createPerformanceDemo(space) {
    // Log demo creation for debugging
    ...
    if (!performancePlugin) {
        // Warn when PerformancePlugin is not found
    ...
    // Log successful demo creation for debugging
...
function createLargeNodeClusters(space) {
    ...
    // Log cluster creation for debugging
}
...
function createRepeatingPatterns(space) {
    ...
    // Log pattern grid creation for debugging
}
...
function createDistanceBasedLOD(space) {
    ...
    // Log LOD demo creation for debugging
}
    ...
    setTimeout(() => {
        ...
        panel.querySelector('#optimize-button')?.addEventListener('click', () => {
            if (space.performance) {
                ...
                // Log manual optimization for debugging
            }
        });
...
function updateWorkerStats(panel) {
    ...
    // Note: 'space' is not accessible in this scope, but this function is called with the right context
    // This is likely a design issue where the space object should be passed as a parameter
    if (!statsElement) return;

    // Since we don't have access to space here, we'll just show a placeholder
    // In a real implementation, space should be passed as a parameter
    statsElement.textContent = 'Worker stats unavailable';
}
...
function addStressTestNodes(space) {
    ...
    // Log stress test node addition for debugging
}
