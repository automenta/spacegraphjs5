...
export class Edge {
  ...
  constructor(id, sourceNode, targetNode, data = {}) {
    ...
    this.data = Utils.mergeDeep({}, Edge.defaultData, data);

    if (this.data.gradientColors?.length === 2) {
      this.data.color = null;
    } else if (this.data.color === null) {
      this.data.color = Edge.defaultData.color;
    }
  ...
  /**
   * Generate gradient colors for the line geometry
   * @private
   */
  _generateGradientColors() {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      color.copy(this._colorStart).lerp(this._colorEnd, t);
  ...
  update() {
    ...
    // Validate positions
    if (
      !this._isValidPosition(sourcePos) ||
      !this._isValidPosition(targetPos)
    ) {
      return;
    }
  ...
  setHighlight(highlight) {
    ...
    const hasGradient =
      this.data.gradientColors?.length === 2 && mat.vertexColors;
