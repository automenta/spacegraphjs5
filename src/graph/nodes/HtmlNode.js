...
export class HtmlNode extends Node {
  ...
  constructor(id, position, data = {}, mass = 1.0) {
    ...
    const initialWidth =
      this.data.width ?? GRAPH_CONSTANTS.DEFAULT_NODE_SIZE * 3.2;
    const initialHeight =
      this.data.height ?? GRAPH_CONSTANTS.DEFAULT_NODE_SIZE * 1.4;
  ...
  _initContentEditable(element) {
    ...
    contentDiv.addEventListener(
      "wheel",
      (e) => this._handleContentWheel(e, contentDiv),
      { passive: false },
    );
  }

  /**
   * Handle wheel event on content
   * @param {WheelEvent} e - Wheel event
   * @param {HTMLElement} contentDiv - Content element
   * @private
   */
  _handleContentWheel(e, contentDiv) {
    const isScrollable =
   ...
   * @param {number} deltaFactor - Scale factor
   */
  ...
  update(space) {
    ...
    if (this.billboard) {
      const camera = space?.plugins
        ?.getPlugin("CameraPlugin")
        ?.getCameraInstance();
      if (camera) {
        this.cssObject.quaternion.copy(camera.quaternion);
      }
    }
  ...
  endResize() {
    ...
    try {
      ...
      console.error(
        `HtmlNode.endResize: Failed to release node ${this.id} during resize:`,
        error,
      );
    }
