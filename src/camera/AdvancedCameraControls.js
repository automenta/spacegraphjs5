...
export class AdvancedCameraControls {
    ...
    _calculateSceneBoundingBox(nodes) {
        ...
        nodes.forEach(node => {
            ...
            nodeBox.setFromCenterAndSize(pos, new THREE.Vector3(radius * 2, radius * 2, radius * 2));
    ...
    _calculateOptimalZoomDistance(boundingBox, nodeCount) {
        ...
        const distance =
            (maxDimension * this.settings.autoZoom.targetPadding) / (2 * Math.tan(fov / 2));
    ...
    _enterPeekMode() {
        this.isPeeking = true;
        if (!this.camera) {
            // Warn when camera is not available for peek mode
    ...
    _updatePeekDirection() {
        ...
        if (!this.camera) {
            // Warn when camera is not available for peek direction update
            // Silently handle in production
            return;
        }
    ...
    smartFocusOnNode(node, options = {}) {
        ...
        if (!node) return;
        if (!node.position) {
            // Warn when node position is undefined for primary node
            // Silently handle in production
            return;
        }
