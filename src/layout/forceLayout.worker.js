...
function _calculateStepInWorker() {
    ...
    edges.forEach(edge => {
        ...
        if (!sourceNode.isFixed)
            nodeForces.set(sourceNode.id, vecAdd(nodeForces.get(sourceNode.id), forceVec));
        if (!targetNode.isFixed)
            nodeForces.set(targetNode.id, vecSub(nodeForces.get(targetNode.id), forceVec));
    });
    ...
    nodes.forEach(node => {
        ...
        currentTotalEnergy +=
            0.5 * (node.mass || 1.0) * (node.vx * node.vx + node.vy * node.vy + node.vz * node.vz);
    });
