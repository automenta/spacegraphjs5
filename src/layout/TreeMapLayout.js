export class TreeMapLayout {
    ...
    _calculateRowRatio(row, rowArea, rectWidth, rectHeight) {
        ...
        // Note: totalArea is not available in this scope, but it's needed for the calculation
        // This suggests a design issue where totalArea should be passed as a parameter
        // For now, we'll use a simplified ratio calculation without totalArea
        const side = Math.min(rectWidth, rectHeight);
        const ratio = (side * rowArea) / minArea;
        const inverseRatio = maxArea / (side * rowArea);
